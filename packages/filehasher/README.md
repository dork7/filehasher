...
Omit `-p` / `--password` to use `.filehasher/password` in the project (if present) or a masked prompt. Interactive passwords are saved to `.filehasher/password` (gitignore this file). Use `--force-prompt` to ignore the stored file.
